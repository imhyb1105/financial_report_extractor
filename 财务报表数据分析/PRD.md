...
## 🆕 V2.14 版本更新 (2026-04-12)

### 更新背景

**问题**: V2.13部署后，测试中国银行360页年报PDF（单位：百万元），所有24项财务指标"最终结果"列全部显示"-"，模型A/B列均无有效数据。

**根因分析**:
1. MiniMax M2.7 模型返回 `unit: "百万元"` 而非 prompt 要求的 `yuan/wan/yi` 标准代码
2. `UnitConvertService.toYuan` 映射仅包含3种标准代码，无法识别"百万元"
3. 转换公式 `value * this.toYuan["百万元"]` → `value * undefined` = `NaN`
4. `convert()` 方法同时转换 `modelResults` 中的数据，导致模型A/B列也全部为 `NaN`
5. 前端 `formatValue(NaN)` 返回 `"-"`，`formatSimpleValue(NaN)` 显示异常

**数据流缺陷链**:
```
  MiniMax M2.7 不遵守 prompt 单位规范
       │
       ▼
  返回 unit="百万元"（PDF原文单位）
       │
       ▼
  UnitConvertService.toYuan["百万元"] = undefined
       │
       ▼
  value * undefined = NaN
       │
       ├──→ financialMetrics 全部 NaN → "最终结果" 列显示 "-"
       ├──→ modelResults.modelA 全部 NaN → "模型A" 列显示异常
       └──→ modelResults.modelB 全部 NaN → "模型B" 列显示异常
```

### 一、解决方案: 单位智能识别与NaN防护

**核心思路**: 扩展 `UnitConvertService` 的单位映射表，覆盖所有中国上市公司年报常见单位；同时增加NaN防护，未知单位时使用智能识别作为后备。

#### 1.1 修复前 vs 修复后 — 财务指标表格
...
```
  ...
  │                                                 Token: 468,463               │
  │ ┌──────────────────┬──────────┬────────────┬────────────┬──────────┐          │
  │ │ 指标名称          │ 最终结果  │ 模型A(MM)  │ 模型B(MM)  │ 置信度   │          │
  │ ├──────────────────┼──────────┼────────────┼────────────┼──────────┤          │
  │ │ 营业收入          │    -     │   解析失败  │   解析失败  │ 高置信度  │          │
  │ │ 营业成本          │    -     │   解析失败  │   解析失败  │ 高置信度  │          │
  │ │ 净利润            │    -     │   解析失败  │   解析失败  │ 高置信度  │          │
  │ │ 总资产            │    -     │   解析失败  │   解析失败  │ 高置信度  │          │
  │ │ ...（全部24项均为-）│    -     │   解析失败  │   解析失败  │ 高置信度  │          │
  │ └──────────────────┴──────────┴────────────┴────────────┴──────────┘          │
  │                                                                                │
  │  ⚠️ 数据异常警告: 模型B解析失败（AI响应格式异常），仅使用模型A的数据            │
  └────────────────────────────────────────────────────────────────────────────────┘
```
...
```
  ...
  │ │ 指标名称          │ 最终结果          │ 模型A(MM)    │ 模型B(MM)  │ 置信度   ││
  │ ├──────────────────┼──────────────────┼──────────────┼────────────┼──────────┤│
  │ │ 营业收入          │ 65,831,000.00 万元│ 65,831,000.00│ 65,831,000│ 中高置信度││
  │ │ 营业成本          │ 35,836,000.00 万元│ 35,836,000.00│     -     │ 高置信度  ││
  │ │ 净利润            │ 25,793,600.00 万元│ 25,793,600.00│ 25,793,600│ 中高置信度││
  │ │ 总资产            │3,835,807,600.00万元│3,835,807,600│3,835,807,600│中高置信度││
  │ │ ...（其余指标正常）│    正常显示       │   正常显示    │   正常显示  │ 正常评级  ││
  │ └──────────────────┴──────────────────┴──────────────┴────────────┴──────────┘│
  │                                                                                │
  │  （原单位: 百万元）                                                              │
  └────────────────────────────────────────────────────────────────────────────────┘
...
```
  ...
  │  toYuan = {                                                   │
  │    // 标准代码 (原有)                                          │
  │    yuan: 1,  wan: 10000,  yi: 100000000,                     │
  │    // 中国上市公司年报常见单位 (新增)                            │
  │    '百万元': 1000000,  '千元': 1000,                          │
  │    '十万元': 100000,   '万': 10000,                           │
  │    '百万': 1000000,    '千': 1000,                            │
  │    '亿': 100000000,    '元': 1,                               │
  │    '万元': 10000,      '亿元': 100000000,                     │
  │    '港元': 1,          'HKD': 1,                              │
  │    ...                                                        │
  │  }                                                            │
  │                                                               │
  │  normalizeUnit(unit):        ← 新增方法                       │
  │    if toYuan[unit] → 直接使用                                 │
  │    if detectUnit(unit) → 智能识别后备                          │
  │    else → 降级为 yuan (安全默认值)                             │
  │                                                               │
  │  convert(metric):                                             │
  │    factor = toYuan[unit] || detectUnit(unit) || 1  ← NaN防护  │
  │    valueInYuan = value * factor                               │
  │    ✅ unit="百万元" → factor=1000000 → 正确转换!               │
  │                                                               │
  │  unitDisplayNames = { ... }    ← 新增: 原单位中文显示名        │
  └──────────────────────────────────────────────────────────────┘
```
...
| 文件 | 变更内容 | 影响范围 |
|------|---------|---------|
| `backend/src/services/UnitConvertService.js` | 扩展 `toYuan` 映射（新增10+单位）；新增 `normalizeUnit()` 方法；`convert()`/`convertMetrics()` 增加 NaN 防护；新增 `unitDisplayNames` 映射 | 仅影响单位转换逻辑 |
| `backend/src/adapters/BaseAdapter.js` | 强化 prompt 中的单位说明，增加单位映射示例和禁用警告 | 仅影响 AI 提取 prompt 文本 |
| `backend/src/app-handler.js` | 版本号 `2.13.0` → `2.14.0` | 仅版本号 |

#### 2.2 前端变更

| 文件 | 变更内容 | 影响范围 |
|------|---------|---------|
| `frontend/src/App.jsx` | Footer 版本号 `V2.13` → `V2.14` | 仅显示文字 |

#### 2.3 不受影响的模块

| 模块 | 说明 |
|------|------|
| `ExtractionService.js` | 调用 `unitConvertService.convert()` 的接口不变 |
| `MiniMaxAdapter.js` | 无变更，adapter 层不涉及单位处理 |
| 其他5个 adapter | 无变更 |
| `store/useStore.js` | 无变更 |
| `ExtractionResult/index.jsx` | 无变更（NaN 防护在服务端解决） |
| `FileUploader/index.jsx` | 无变更 |
| `apiService.js` | 无变更 |
| 所有 API 路由 | 无变更 |
| 数据库 | 无变更 |
...
this.unitDisplayNames = {
  yuan: '元', wan: '万元', yi: '亿元',
  '百万元': '百万元', '千元': '千元', '十万元': '十万元',
  '万': '万', '亿': '亿', '百万': '百万', '千': '千',
  '港元': '港元', '千万元': '千万元'
}
```

**3.1.3 convert() 方法增加 NaN 防护**

```javascript
// 修复前:
const valueInYuan = originalValue * this.toYuan[originalUnit]  // 可能 NaN

// 修复后:
const factor = this.toYuan[originalUnit] || this.detectUnitFactor(originalUnit) || 1
const valueInYuan = originalValue * factor
```

**3.1.4 新增 detectUnitFactor() 方法**

利用已有的 `detectUnit()` 方法，将智能识别结果转为转换系数：

```javascript
detectUnitFactor(unitStr) {
  const detected = this.detectUnit(unitStr)
  return this.toYuan[detected] || null
}
```

#### 3.2 BaseAdapter.js Prompt 优化

在"单位说明"部分增加映射示例和禁用警告：
...
| 风险 | 级别 | 应对措施 |
|------|------|---------|
| 已有数据单位为 yuan/wan/yi 的不受影响 | 无风险 | 扩展映射只新增键，不修改原有键值 |
| 单位智能识别误判 | 极低 | detectUnit 只在 toYuan 直接查找失败时才触发，作为后备 |
| prompt 优化后 AI 仍不遵守 | 极低 | 后端单位扩展是主要防线，prompt 优化是次要防线 |
| 前端不需要任何代码改动 | 无风险 | NaN 防护在服务端完成，前端接收到的就是正确数值 |

---

## 🆕 V2.13 版本更新 (2026-04-11)

### 更新背景

**问题**: V2.12部署后，测试360页中国银行年报PDF时出现 **504 Gateway Timeout** 错误。总提取时长305秒，超过Vercel serverless function的300秒maxDuration限制。

**根因分析**:
1. 三模型全流程(财务提取→验证→非财务提取→非财务总结)共5-6次串行/并行API调用
2. V2.11将MiniMax max_tokens从8192提升至16384，单次API调用耗时显著增加(单次最长~120s)
3. 非财务信息提取(3次API调用: A/B并行 + C总结)叠加在财务提取流程之后，导致总时长溢出

**时间线分析(360页PDF)**:
```
  0s ──────── 120s ────── 180s ────── 240s ────── 305s ──→ 504!
  │ Model A/B │ Model C  │ 非财务A/B │ 非财务C   │
  │ 财务提取  │ 验证     │ 并行提取  │ 总结     │
  └───────────┴──────────┴───────────┴──────────┘
                            ↑ 300s Vercel限制
```
...
**核心思路**: 将非财务信息提取从主流程中拆分为独立API，两次请求各自在300s内完成。

```
  请求1: 财务数据提取 (预计 ~180s)
  ┌──────────────────┬──────────────┐
  │ Model A/B 并行   │ Model C      │
  │ 财务提取         │ 验证         │
  └──────────────────┴──────────────┘
  → 立即返回财务结果给前端
...
| 文件 | 变更内容 |
|------|---------|
| `backend/src/services/ExtractionService.js` | 移除`_processExtraction`中的`enhanceNonFinancialInfo`调用；新增`extractNonFinancialFromText()`独立方法 |
| `backend/src/routes/extract.js` | 新增`POST /api/extract/non-financial`路由 |
| `backend/src/app-handler.js` | 版本号更新为2.13.0 |

#### 2.2 前端变更

| 文件 | 变更内容 |
|------|---------|
| `frontend/src/services/apiService.js` | 新增`extractNonFinancial()`API；`extractData()`返回值增加`pdfResult`字段 |
| `frontend/src/components/FileUploader/index.jsx` | 财务结果返回后异步调用非财务提取 |
| `frontend/src/store/useStore.js` | 新增`setNonFinancialInfo`、`isNonFinancialLoading`、`setNonFinancialLoading`状态 |
| `frontend/src/components/ExtractionResult/index.jsx` | 非财务信息Tab增加加载中/空状态提示 |
| `frontend/src/App.jsx` | 版本号更新为V2.13 |
...
| 模块 | 是否影响 | 说明 |
|------|---------|------|
| 财务数据提取 | ❌ 无影响 | 主流程不变，仅移除非财务提取步骤 |
| 勾稽关系核对 | ❌ 无影响 | 不涉及非财务信息 |
| 置信度计算 | ❌ 无影响 | 置信度仅基于财务数据 |
| AI调试日志 | ❌ 无影响 | 非财务提取的日志在独立请求中记录 |
| 小文件(<4MB) | ⚠️ 轻微 | pdfResult为null，不触发非财务异步调用(小文件无超时风险) |
| 单/双模型 | ❌ 无影响 | 非财务提取仅在三模型模式下触发 |

---

## 🆕 V2.12 版本更新 (2026-04-11)

### 更新背景

**问题**: V2.11修复后，MiniMax M2.7已能成功提取数据，但存在两个严重问题：

1. **虚假高置信度**: Model A提取25项指标(含6项非预期比率)，Model B提取10项(含4项非预期银行指标)。4项核心指标(流动资产/流动负债/应收账款/存货)为null。Model C给出4/5分+勾稽核对+1=**5分(高置信度)**，用户看到"高置信度"但数据明显不完整。

2. **非预期指标混入**: 模型自行添加不在prompt要求列表内的指标(如不良贷款率、核心一级资本充足率、利息净收入、基本每股收益)，混入最终结果。

**根因分析**:
1. `adjustConfidenceFromScore()`只考虑Model C评分+勾稽核对，**完全不考虑指标完整性**
2. `ExtractionService._processExtraction()`不过滤非预期指标
3. 前端对null值指标没有醒目提示
4. Model C验证prompt不告知预期指标列表，无法评判"缺失"
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│               V2.12 adjustConfidenceFromScore() 修复                        │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  修改前:                                          修改后:                    │
│  ┌───────────────────────────────┐        ┌──────────────────────────────┐│
│  │ modelCScore = 4               │        │ modelCScore = 4              ││
│  │ accountingCheck → +1          │        │ accountingCheck → +1         ││
│  │ finalScore = 4+1 = 5 (高)     │        │                               ││
│  │                               │        │ [NEW] 指标完整性检查:         ││
│  │ ❌ 不考虑null值               │        │ nullCount = 4 (流动资产等)    ││
│  │ ❌ 不考虑非预期指标           │        │ nullRatio = 4/19 = 21%        ││
│  │ ❌ 不考虑预期指标缺失         │        │ nullPenalty = -1              ││
│  └───────────────────────────────┘        │                               ││
│                                           │ [NEW] 非预期指标惩罚:          ││
│                                           │ extraCount = 0 (过滤后)       ││
│                                           │                               ││
│                                           │ finalScore = 4+1-1 = 4 (中高) ││
│                                           └──────────────────────────────┘│
│                                                                             │
│  置信度扣分规则:                                                             │
│  ┌──────────────────────────────────────────────────────────────────┐      │
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ExtractionService._processExtraction()                                     │
│  ┌──────────────────────────────────────────────────────────────────┐      │
│  │                                                                  │      │
│  │  result.financialMetrics = [                                     │      │
│  │    {name:"营业收入",value:658310},  ← 预期，保留                │      │
│  │    {name:"不良贷款率",value:1.23},  ← 非预期，过滤              │      │
│  │    {name:"毛利率",value:0.4556},    ← 计算指标，保留(服务端重算)│      │
│  │    {name:"核心一级资本充足率",...}  ← 非预期，过滤              │      │
│  │  ]                                                               │      │
│  │                                                                  │      │
│  │  [NEW] this.filterExpectedMetrics(result)                        │      │
│  │                                                                  │      │
│  │  EXPECTED_METRICS = [                                            │      │
│  │    '营业收入','营业成本','毛利润','净利润','归母净利润',         │      │
│  │    '少数股东损益','扣非净利润','总资产','总负债',                │      │
│  │    '归母所有者权益','少数股东权益','所有者权益合计',             │      │
│  │    '流动资产','流动负债','应收账款','存货',                      │      │
│  │    '经营现金流','投资现金流','筹资现金流',                        │      │
│  │    '毛利率','净利率','资产负债率','流动比率','ROE'               │      │
│  │  ]                                                               │      │
│  │                                                                  │      │
│  │  过滤后: 保留预期列表内的指标 + 添加指标完整度统计               │      │
│  │  result.metricStats = {                                          │      │
│  │    expected: 19,                                                 │      │
│  │    extracted: 15,                                                │      │
│  │    nullValues: 4,                                                │      │
...
│  提取结果卡片头部:                                                           │
│  ┌──────────────────────────────────────────────────────────────────┐      │
│  │ 提取结果  [中高置信度] [指标完整度: 79% (15/19)]    [调试][导出] │      │
│  │                                                                  │      │
│  │ ⚠️ 以下指标在PDF中未找到: 流动资产, 流动负债, 应收账款, 存货    │      │
│  └──────────────────────────────────────────────────────────────────┘      │
│                                                                             │
│  指标表格中的null值:                                                         │
│  ┌────┬──────────┬──────────┬────────┬────────┐                            │
...
| 文件 | 修改类型 | 修改内容 |
|------|---------|---------|
| `backend/src/services/ExtractionService.js` | 修改 | 新增`filterExpectedMetrics()`，修复`adjustConfidenceFromScore()`加入null值扣分 |
| `backend/src/components/ExtractionResult/index.jsx` | 修改 | 新增指标完整度显示、null值醒目提示 |
| `backend/src/app-handler.js` | 修改 | 版本号升级到2.12.0 |
| `prd.md` | 修改 | 新增V2.12版本说明 |

### 三、不影响非相关模块的保证

1. `filterExpectedMetrics()` 仅在 `_processExtraction()` 中调用，不影响`triModelExtract()`的内部逻辑
2. `adjustConfidenceFromScore()` 仅在置信度已是数字(1-5)时生效，字符串格式走原路径不变
3. 前端改动仅影响 `ExtractionResult` 组件的显示逻辑，不改变数据结构
4. 不影响其他模型适配器（豆包、智谱等）
5. 不影响双模型/单模型模式

## 🆕 V2.11 版本更新 (2026-04-11)

### 更新背景

**问题**: MiniMax M2.7 模型提取360页中国银行年报时，数据提取严重不完整：
- 模型A仅提取5项指标（其中3项不在要求列表内）
- 模型B返回JSON被截断，只解析出部分数据
- 模型C验证阶段JSON也被截断
- 所有API调用的`finishReason`均为`undefined`

**根因分析**:
1. **模型提前停止**: `completion_tokens`远低于`max_tokens`(8192)，实际使用651~1971 tokens
2. **不遵循prompt**: 模型自行决定提取银行特有指标（不良贷款率），忽略要求的21项标准指标
3. **截断JSON无法恢复**: 现有`extractJsonObject()`括号深度匹配在找不到闭合`}`时直接返回null
4. **Reasoning浪费tokens**: 部分调用中模型将tokens用于内部推理(534 reasoning_tokens)，不产出结构化JSON
...
│  ┌──────────────────────────────────────────┐                              │
│  │ rawText: "{ \"companyInfo\": { ... },    │                              │
│  │   \"financialMetrics\": [ { \"name\":    │                              │
│  │   \"营业收入\", \"value\": 658310 },      │  ← 截断的JSON                │
│  │   { \"name\": \"净利润\", \"value\":       │                              │
│  └──────────────────────────────────────────┘                              │
│          │                                                                  │
│          ▼                                                                  │
│  ┌──────────────────────────────────────────────────────────────────┐      │
│  │              BaseAdapter.parseResponse() V2.11                   │      │
│  ├──────────────────────────────────────────────────────────────────┤      │
│  │                                                                  │      │
│  │  Step 1: 剥离伪标签 (tool_call等)                                  │      │
│  │  Step 2: 提取JSON (代码块 → 括号匹配)                              │      │
│  │  Step 3: 直接JSON.parse → 成功? → 返回                            │      │
│  │  Step 4: sanitize后JSON.parse → 成功? → 返回                      │      │
│  │  Step 5: relaxedJsonParse → 成功? → 返回                          │      │
│  │  Step 6: [NEW] 截断JSON修复 → repairTruncatedJson() → 返回        │      │
│  │          ├─ 找到最后一个完整的对象/数组元素                          │      │
│  │          ├─ 补全未闭合的括号                                       │      │
│  │          └─ 解析修复后的JSON                                       │      │
│  │  Step 7: 返回 getDefaultResult()                                  │      │
│  │                                                                  │      │
│  └──────────────────────────────────────────────────────────────────┘      │
│          │                                                                  │
│          ▼                                                                  │
...
│  │              [NEW] 指标完整性验证                                 │      │
│  ├──────────────────────────────────────────────────────────────────┤      │
│  │  EXPECTED_METRICS = [营业收入, 营业成本, 净利润, 归母净利润, ...]  │      │
│  │                                                                  │      │
│  │  extractedNames = result.financialMetrics.map(m => m.name)        │      │
│  │  missingMetrics = EXPECTED_METRICS.filter(n => !extractedNames)  │      │
│  │                                                                  │      │
│  │  if (missingMetrics.length > 5) {                                │      │
│  │    result.metricWarning = `缺少${missingMetrics.length}项指标`    │      │
│  │  }                                                               │      │
│  └──────────────────────────────────────────────────────────────────┘      │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
#### 1.2 MiniMax API调用参数优化

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    MiniMaxAdapter V2.11 参数优化                              │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  修改前:                          修改后:                                    │
│  ┌──────────────────────┐        ┌──────────────────────┐                  │
...
│  │ (无额外指令)          │   →    │ + system prompt引导  │                  │
│  └──────────────────────┘        └──────────────────────┘                  │
│                                                                             │
│  system prompt 内容:                                                         │
│  ┌──────────────────────────────────────────────────────────────────┐      │
│  │ "你是一个精确的财务数据提取助手。                                  │      │
│  │  请直接输出JSON，不要输出任何推理过程、分析说明或其他文字。        │      │
│  │  不要使用<minimax:tool_call>标签。                                │      │
│  │  确保输出的JSON格式完整，所有括号正确闭合。"                       │      │
│  └──────────────────────────────────────────────────────────────────┘      │
│                                                                             │
│  validate方法: max_tokens: 4096 → 8192                                      │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### 1.3 ExtractionService 容错流程优化

```
┌─────────────────────────────────────────────────────────────────────────────┐
│              ExtractionService triModelExtract V2.11 流程                    │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌──────────┐    ┌──────────┐                                              │
│  │ Model A  │    │ Model B  │    并行提取                                   │
│  │ extract  │    │ extract  │                                              │
│  └────┬─────┘    └────┬─────┘                                              │
│       │               │                                                    │
│       ▼               ▼                                                    │
│  ┌─────────────────────────────────────────────────────────┐               │
│  │ [NEW] 指标完整性检查                                      │               │
│  │ hasAMetrics = A.financialMetrics.length > 0              │               │
│  │ hasBMetrics = B.financialMetrics.length > 0              │               │
│  │ aMetricCount = A.financialMetrics.length                  │               │
│  │ bMetricCount = B.financialMetrics.length                  │               │
│  │ MIN_METRICS = 8  (21项中的最低要求)                        │               │
│  │                                                          │               │
│  │ if (aMetricCount > 0 && aMetricCount < MIN_METRICS) {    │               │
│  │   A.metricWarning = "指标不完整"                           │               │
│  │   A.confidence = 'low'                                    │               │
│  │ }                                                        │               │
│  └─────────────────────────────────────────────────────────┘               │
│       │               │                                                    │
...
│  ┌─────────────────────────────────────────────────────────┐               │
│  │ 原有逻辑:                                                │               │
│  │ • 双模型都失败 → 合并空结果, confidence=low               │               │
│  │ • 单模型成功 → 使用成功模型, confidence=medium-high       │               │
│  │ • 双模型成功 → Model C裁决                               │               │
│  │                                                          │               │
│  │ [NEW] 改进:                                              │               │
│  │ • 单模型成功但指标<MIN → 标记warning，仍使用              │               │
│  │ • 双模型都成功但指标都不足 → 选指标多的那个                │               │
│  └─────────────────────────────────────────────────────────┘               │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| 文件 | 修改类型 | 修改内容 |
|------|---------|---------|
| `backend/src/adapters/BaseAdapter.js` | 修改 | 新增`repairTruncatedJson()`截断JSON修复方法 |
| `backend/src/adapters/MiniMaxAdapter.js` | 修改 | 增大max_tokens，添加system prompt |
| `backend/src/services/ExtractionService.js` | 修改 | 增强指标完整性检查逻辑 |
| `backend/src/app-handler.js` | 修改 | 版本号升级到2.11.0 |
| `prd.md` | 修改 | 新增V2.11版本说明 |

### 三、技术要点

1. **截断JSON修复**: `repairTruncatedJson()` 算法
   - 找到最后一个完整的JSON值（通过定位最后的`},` 或 `],` 模式）
   - 截断到该位置，补全缺失的 `]` 和 `}`
   - 即使只能恢复部分指标也比全部丢弃好

2. **MiniMax M2.7 system prompt**: 通过system角色消息引导模型行为
   - 减少内部推理输出
   - 避免tool_call伪标签
   - 确保JSON输出完整

3. **最小指标阈值**: 设置MIN_METRICS=8（21项中的约40%）
   - 低于此阈值的提取结果被标记为`metricWarning`
   - 但仍然返回已提取的指标（部分结果优于空结果）

4. **向后兼容性**: 所有修改仅影响MiniMaxAdapter和BaseAdapter的内部逻辑
   - 不改变API接口
   - 不影响其他模型适配器（豆包、智谱、通义千问、DeepSeek）
   - 前端无需改动
