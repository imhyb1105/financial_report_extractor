...
| **2.1** | **2026-03-26** | **🆕 新增三层记忆系统 (Three-Layer Memory System): L1全局记忆、L2项目会话记忆、L3会话快照；防止对话中断导致进度丢失；定义自动保存触发点和恢复流程** |
...
## 🧠 Three-Layer Memory System (三层记忆系统)

> **目的**: 防止对话意外中断导致开发进度丢失，确保开发进程连续性

### 架构概览

```
┌─────────────────────────────────────────────────────────────────┐
│                    L1: 全局记忆 (Global Memory)                   │
│  位置: ~/.claude/projects/{project-hash}/memory/MEMORY.md        │
│  内容: 项目概述、技术栈、关键路径、用户偏好                          │
│  更新: 每个版本发布后 / 重大架构变更时                              │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                    L2: 项目会话记忆 (Project Session)             │
│  位置: {project}/.claude/session/SESSION.md                      │
│  内容: 当前版本开发状态、待办任务、已解决问题、下一步行动            │
│  更新: 每次对话结束时 / 完成关键任务后                              │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                    L3: 会话快照 (Session Snapshot)                │
│  位置: {project}/.claude/session/SNAPSHOT_{timestamp}.md         │
│  内容: 详细开发记录、代码变更、测试结果、问题分析                    │
│  更新: 每次重要操作后自动保存                                       │
└─────────────────────────────────────────────────────────────────┘
```
...
├── SESSION.md                   # L2: 项目会话记忆
└── SNAPSHOT_*.md                # L3: 会话快照 (多个)
```
...
║  触发事件                              更新文件        更新内容            ║
║  ─────────────────────────────────────────────────────────────────────   ║
║  完成一个开发阶段 (PHASE)              L2 SESSION.md   进度追踪表          ║
║  解决一个 bug                          L2 SESSION.md   已解决问题表        ║
║  开始新任务                            L2 SESSION.md   待办任务            ║
║  完成测试 (L0-L6)                      L2 SESSION.md   测试结果            ║
║  L7 人工测试通过/失败                  L2 + L3         详细记录            ║
║  L8 人工验收通过/失败                  L2 + L3         验收结果            ║
║  代码变更 (新增/修改文件)              L3 SNAPSHOT     变更记录            ║
║  发现新问题                            L2 SESSION.md   待解决问题          ║
║  **对话即将结束**                      L2 + L3         完整状态            ║
║  **用户请求保存进度**                  L2 + L3         完整状态            ║
║                                                                           ║
╚═══════════════════════════════════════════════════════════════════════════╝
...
| 阶段 | 状态 | 完成时间 | 备注 |
|------|------|----------|------|
| PHASE 0-3 | ✅/⏸️/❌ | {date} | {note} |
| L0-L6 测试 | ✅/⏸️/❌ | {date} | {note} |
| L7 人工测试 | ✅/⏸️/❌ | {date} | {note} |
| L8 人工验收 | ✅/⏸️/❌ | {date} | {note} |
| 部署 | ✅/⏸️/❌ | {date} | {note} |
...
对话意外中断后恢复:
    │
    ├─ Step 1: 读取 L1 (MEMORY.md)
    │           ↓ 了解项目整体情况
    │
    ├─ Step 2: 读取 L2 (SESSION.md)
    │           ↓ 了解当前开发进度
    │
    ├─ Step 3: 读取最新的 L3 快照
    │           ↓ 了解详细操作记录
...
```
PHASE 0: 初始化
    └─ 创建 .claude/session/ 目录
    └─ 初始化 SESSION.md

PHASE 4: 测试与修复
    └─ L0-L6 测试完成后 → 更新 SESSION.md
    └─ L7 人工测试每项完成后 → 更新 SESSION.md
    └─ 发现问题时 → 创建 SNAPSHOT
    └─ 问题修复后 → 更新 SESSION.md

L8: 人工验收
    └─ 验收完成后 → 更新 SESSION.md + 创建 SNAPSHOT

PHASE 5: 部署
    └─ 部署完成后 → 更新 SESSION.md + 创建 SNAPSHOT
    └─ 更新 L1 MEMORY.md (版本信息)
...
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                           ║
║  1. 每个 PHASE 完成后 MUST 更新 SESSION.md                                ║
║  2. L7/L8 测试每项完成后 MUST 更新 SESSION.md                              ║
║  3. 对话结束前 MUST 创建/更新 SESSION.md                                   ║
║  4. 重大变更后 MUST 创建 SNAPSHOT                                          ║
║  5. 用户请求"保存进度"时 MUST 立即执行                                      ║
║                                                                           ║
║  忽略记忆更新的后果:                                                       ║
║  - 对话中断后无法恢复进度                                                  ║
║  - 重复已完成的工作                                                        ║
...
*SKILL.md v2.1 - Added Three-Layer Memory System (2026-03-26)*
