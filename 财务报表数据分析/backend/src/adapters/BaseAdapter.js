...
- wan: 万元（PDF中显示"单位：万元"时使用）
- yi: 亿元（PDF中显示"单位：亿元"时使用）

### 单位映射规则（必须遵守）：
- PDF显示"单位：百万元" → 使用 yi，value 需除以 100（如 658310百万元 → value=6583.10, unit=yi）
- PDF显示"单位：千元" → 使用 wan，value 需乘以 10（如 12345千元 → value=123450, unit=wan）
- PDF显示"单位：十万元" → 使用 wan，value 需乘以 100
- PDF显示"单位：千万元" → 使用 yi，value 需乘以 10

⚠️ 绝对不要在 unit 字段填写"百万元"、"千元"等非标准值！
⚠️ 绝对不要使用拼音！如"baiwan"、"qianwan"、"shiwan"等都不允许！
⚠️ unit 字段只能是 yuan、wan、yi 三者之一！
